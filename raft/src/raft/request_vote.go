...
func (rf *Raft) RequestVote(args *RequestVoteArgs, reply *RequestVoteReply) {
	...
	if args.Term > rf.currentTerm { // 发来最新的消息
		rf.setNewTerm(args.Term)
	}

	if args.Term < rf.currentTerm { // 发来的消息不如当前消息新
	...
	if *voteCounter > len(rf.peers) / 2 && 
		...
		rf.state == Candidate {
			...
			becomeLeader.Do(func(){
				Dprintf("[%d]: 当前term %d 结束", rf.me, rf.currentTerm)
				rf.state = Leader
